    ...
    public class GibScript : MonoBehaviour
    {
        ...
        private void Start()
        {
            rigidbody.AddForce(
                Vector3.up
                + new Vector3(
                    (Random.value * (m_fXMaxForce - m_fXMinForce) + m_fXMinForce),
                    (Random.value * (m_fYMaxForce - m_fYMinForce) + m_fYMinForce),
                    (Random.value * (m_fZMaxForce - m_fZMinForce) + m_fZMinForce)),
                ForceMode.Acceleration);

            transform.localScale = new Vector3(
                Random.value * (m_fXMaxScale - m_fXMinScale) + m_fXMinScale,
